...
private:
    ...
    // ALSA 相关成员
    snd_pcm_t *capture_handle;          // 录音 PCM 句柄
    snd_pcm_t *playback_handle;         // 播放 PCM 句柄
    std::thread capture_thread;          // 录音线程
    std::thread playback_thread;         // 播放线程
    std::atomic<bool> capture_running;   // 录音运行标志
    std::atomic<bool> playback_running;  // 播放运行标志

    // 内部辅助函数
    void capture_loop();                 // 录音线程循环
    void playback_loop();                // 播放线程循环
    int set_pcm_params(snd_pcm_t *pcm, snd_pcm_stream_t stream,
                       unsigned int rate, unsigned int channels,
                       snd_pcm_format_t format);  // 设置 PCM 硬件参数
