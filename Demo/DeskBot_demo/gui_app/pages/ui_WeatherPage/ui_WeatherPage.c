...
static void _cloud_move_anim(void)
{
    ...
    if (ui_ImgCloud == NULL) {
        LV_LOG_WARN("ui_ImgCloud is NULL, skip animation");
        return;
    }
...
static void _ui_event_Gesture(lv_event_t * e)
{
...
void ui_WeatherPage_init(void)
{
    ...
    lv_obj_set_width(ui_LabelsPanel, 600);
    lv_obj_set_height(ui_LabelsPanel, 480);
    ...
    // event
    lv_obj_add_event_cb(ui_WeatherPage, _ui_event_Gesture, LV_EVENT_ALL, NULL);
    ...
    // debug log
    LV_LOG_USER("ui_WeatherPage_init success\n");
}
