...
static void ui_canvas_event(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_PRESSING)
    {
        ...
        // 将触摸点坐标转换为相对于画布的坐标
        lv_coord_t canvas_x = point.x - canvas_coords.x1;
        lv_coord_t canvas_y = point.y - canvas_coords.y1;

        // 确保坐标在画布范围内
        if(canvas_x < 0) canvas_x = 0;
        if(canvas_y < 0) canvas_y = 0;
        if(canvas_x >= UI_SCREEN_WIDTH) canvas_x = UI_SCREEN_WIDTH - 1;
        if(canvas_y >= UI_SCREEN_HEIGHT) canvas_y = UI_SCREEN_HEIGHT - 1;
