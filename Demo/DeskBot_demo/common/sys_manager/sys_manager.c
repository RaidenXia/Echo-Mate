...
int sys_set_lcd_brightness(int brightness) {
    ...
    if (brightness < 50) brightness = 50; // 亮度太低可能导致屏幕无法显示
    if (brightness > 95) brightness = 95;

    int max_brightness = 7;
    FILE *fp = fopen("/sys/class/backlight/backlight/max_brightness", "r");
    if (fp) {
    ...
    // 将百分比映射到硬件值
    int hardware_value = brightness * max_brightness / 100;
    // 确保值在有效范围内
    if (hardware_value < 0) hardware_value = 5;
    if (hardware_value > max_brightness) hardware_value = max_brightness;
    ...
    int n = snprintf(buffer, sizeof(buffer), "%d", hardware_value);
...
int sys_get_day_of_week(int year, int month, int day) {
    ...
    int dayOfWeek = f % 7; // 0=星期六, 1=星期日, 2=星期一, 3=星期二, 4=星期三, 5=星期四, 6=星期五

    // 调整到常见一周起始：0=星期日, 1=星期一, ..., 6=星期六
...
bool is_internet_reachable(void) {
    ...
    servaddr.sin_port = htons(53); // ali DNS服务端口
    servaddr.sin_addr.s_addr = inet_addr("223.5.5.5"); // ali Public DNS IP地址
...
const char* sys_get_city_name_by_adcode(const char *filepath, const char *target_adcode) {
    ...
    // 如果找到结果，创建一个安全的副本，因为原字符串会在json_object_put后失效
    char *safe_result = NULL;
    if (result != NULL) {
        safe_result = strdup(result);  // 创建字符串副本
    }

    // 释放 JSON 对象
    json_object_put(root);
    
    return safe_result;  // 返回安全的副本
}
...
int sys_get_auto_location_by_ip(LocationInfo_t* location, const char *api_key) {
    ...
    if (!api_key || strlen(api_key) == 0) {
        fprintf(stderr, "API key is invalid\n");
        return -1;
    }
    ...
    json_bool result_province = json_object_object_get_ex(parsed_json, "province", &province_obj);
    json_bool result_city = json_object_object_get_ex(parsed_json, "city", &city_obj);
    json_bool result_adcode = json_object_object_get_ex(parsed_json, "adcode", &adcode_obj);

    // 检查province, city, adcode是否为空数组
    if(result_province && json_object_is_type(province_obj, json_type_array) && json_object_array_length(province_obj) == 0 &&
       result_city && json_object_is_type(city_obj, json_type_array) && json_object_array_length(city_obj) == 0 &&
       result_adcode && json_object_is_type(adcode_obj, json_type_array) && json_object_array_length(adcode_obj) == 0) {
        
        fprintf(stderr, "Location information is empty. This might be due to an invalid or foreign IP address.\n");
    ...
    // 安全地获取城市名称
    if (result_city && city_obj && json_object_is_type(city_obj, json_type_string)) {
        const char* city_value = json_object_get_string(city_obj);
        // fprintf(stderr, "[DEBUG] City value: %s\n", city_value ? city_value : "NULL");
        if (city_value) {
            strncpy(location->city, city_value, sizeof(location->city) - 1);
        } else {
            strcpy(location->city, "Unknown");
        }
    } else {
        fprintf(stderr, "[DEBUG] City object not found or not string type, setting to Unknown\n");
        strcpy(location->city, "Unknown");
    }
    location->city[sizeof(location->city) - 1] = '\0'; // 确保字符串以null结尾
    
    // 安全地获取区域码
    if (result_adcode && adcode_obj && json_object_is_type(adcode_obj, json_type_string)) {
        const char* adcode_value = json_object_get_string(adcode_obj);
        // fprintf(stderr, "[DEBUG] Adcode value: %s\n", adcode_value ? adcode_value : "NULL");
        if (adcode_value) {
            strncpy(location->adcode, adcode_value, sizeof(location->adcode) - 1);
        } else {
            strcpy(location->adcode, "000000");
        }
    } else {
        fprintf(stderr, "[DEBUG] Adcode object not found or not string type, setting to 000000\n");
        strcpy(location->adcode, "000000");
    }
...
int sys_get_time_from_ntp(const char* ntp_server, int *year, int *month, int *day,
    ...
    for (rp = res; rp != NULL; rp = rp->ai_next) {
        ...
        // 构造 NTP 请求包（48字节，RFC 5905）
        buf[0] = 0x1b;  // LI=0, Version=3, Mode=3 (Client)

        // 发送请求
        ssize_t sent = sendto(sockfd, buf, sizeof(buf), 0, rp->ai_addr, rp->ai_addrlen);
        if (sent != sizeof(buf)) {
            close(sockfd);
            sockfd = -1;
            continue;
        ...
        // 接收响应
        ssize_t recvd = recvfrom(sockfd, buf, sizeof(buf), 0, NULL, NULL);
        if (recvd >= sizeof(buf)) {
            // 成功收到完整 NTP 包，跳出循环
            break;
        }
    ...
    if (sockfd == -1) {
        fprintf(stderr, "Failed to communicate with google server\n");
        return -1;
    }

    // 解析时间戳
    uint32_t secs_since_1900;
    memcpy(&secs_since_1900, &buf[40], 4);
    secs_since_1900 = ntohl(secs_since_1900);
    // 转换为 Unix 时间戳
